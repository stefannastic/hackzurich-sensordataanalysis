...
case class BatteryLotReadingJSON4S(id: Int, date_observed: String, total_spot_number: Int, free_slot_number: Int, location_coordinates: String, location_type: String, sensortype: String = "BatteryLot") extends Ordered[BatteryLotReadingJSON4S] {
  def toCsv: String = s"$id ; $date_observed ; $total_spot_number ; $free_slot_number ; $location_coordinates ; $location_type"
...
object BatteryLotReadingJSON4S {
  ...
  def apply(csv: String): BatteryLotReadingJSON4S = {
    try {
      ...
      BatteryLotReadingJSON4S(_id, _dateObserved, _totalSpotNumber, _freeSlotNumber, _locationCoordinates, _locationType )
    } catch {
